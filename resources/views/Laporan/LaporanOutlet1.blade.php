...
<div class="wrapper wrapper-content animated fadeInRight">
    <div class="row">
      <div class="row">
        ...
        <h1><center>Laporan Penjualan Ayam Geprek Pangeran Cabang Dago</center></h1>
            ...
            <table class="table table-striped table-bordered table-hover dataTables-example" >
              <thead>
                              ...
                              <tbody>
                                ...
                                $total=0;
                                @endphp
                                @foreach($so_hdr as $so)
                                <tr>
                                  <td>{{ $so->so_num }}</td>
                                  <td>{{ $so->customer }}</td>
                                  <td>{{ $so->so_date }}</td>
                                  <td>Rp. {{ number_format($so->grand_total,0)}} ,00</td>
                                  <td><a href="{{ route('LaporanAllOutlet.show',$so->so_num) }}" class="btn btn-info btn-xs"><i class="fa fa-print"> Print Detail</i></a></td>
                                  @php
